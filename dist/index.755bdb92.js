window.dom = {
    ...
    attr (node, name, value) {
        //重载：根据参数个数写不同的代码
        //用于读属性
        if (arguments.length === 3) node.setAttribute(name, value); //set
        else if (arguments.length === 2) return node.getAttribute(name); //读取
    },
    //用于读写文本内容
    text (node, string) {
    ...
    html (node, string) {
        //重载：根据参数的长度实现不同的效果
        if (arguments.length === 2) node.innerHTML = string; //写HTML内容
        else if (arguments.length === 1) return node.innerHTML; //读HTML内容
    },
    //用于修改style
    style (node, name, value) {
        if (arguments.length === 3) //dom.style(div, 'color', 'red' ) 设置style
        node.style[name] = value;
        else if (arguments.length === 2) {
            if (typeof name === "string") //dom.style(div,'color') 读取div的color
